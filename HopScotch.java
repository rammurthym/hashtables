...
public class HopScotch<T> {
    ...
    int find(T x) {
        ...
        int limit = ((s0+k) < maxSize) ? s0+k : (s0+k) - (s0+k)%maxSize;

        for (int i = s0; i < limit; i++) {
    ...
    public boolean add(T x) {
        ...
        for (int i = s0; i < limit; i++) {
            int check = movable(i);
            if (check > 0) {
                table[check].element = table[i].element;
                table[check].occupied = true;
                size++;
                return true;
            }
        }
    ...
    public int movable(int index) {
        int limit = ((index+k) < maxSize) ? index+k : (index+k) - (index+k)%maxSize;
    ...
    void resize() {
        resizeCalled++;
        HopScotch<T> temp = new HopScotch<>(maxSize*4);
    ...
    public static void main (String[] args) {
        if (args.length == 2) {
            ...
            if (args[0].equals("0")) {
                ...
                HopScotch<Integer> ht = new HopScotch<>();
                ...
                t1.end();
                System.out.println("Time taken to add " + args[1] + " million elements in HopScotch hashing table: " + t1);

                Timer t2 = new Timer();
                t2.start();
                ht.remove(randomNumberGenerator(0, n));
                t2.end();
                System.out.println("Time taken to remove a random element from HopScotch hashing table: " + t2);
